...
def convert_url_to_csv(url):
    ...
    if (r.status_code // 100 != 2):
        sg.popup_error(str(r.status_code) + " HTTP error: Please try a different URL")
        return 
    ...
    if (urlparse(url).netloc == "arkansasrazorbacks.com") or (urlparse(url).netloc == "vucommodores.com"):
        ...
        if not athletes:
            sg.popup_error("Please double check your URL")
            return
            
        sg.popup("Loading: Please do not exit application")
    ...
    # All of other WMT websites in the hashmap, which do have the athlete images on the main roster
        ...
        if teams:
            ...
            sg.popup_error("Unable to process data from dynamically generated Sidearm URL")
            return
    
    else:
       sg.popup_error("Unable to process data from this roster URL")
       return
...
def save_file(team_url, os_path, csv_file_name):
    team_url = team_url
    df = convert_url_to_csv(team_url)
    if df is not None:
        if len(df.index) > 0:
...
def display_csv_file(team_url, csv_file_name):
    ...
    if df is not None:
        if len(df.index) > 0:
            ...
            if not filename:
                filename = generate_file_name(team_url)
            sg.popup_scrolled(df.to_csv(), title=filename)
...
def is_valid_url(team_url):
    if validators.url(team_url):
        ...
        if any(char.isdigit() for char in split[-1]):
            ...
            if "roster" == split[-1]:
                return True
    sg.popup_error("Please enter a valid team roster URL")
    return False
...
def main_window():
    ...
    layout = [
            ...
            [sg.B("Quick Look"), sg.B("Save"), sg.B("Exit", button_color="tomato")]
    ]
    window_title = "Team Roster URL To CSV Converter"
    window = sg.Window(window_title, layout, alpha_channel=0.9)

    """layout = [
            [sg.T("Enter a team roster URL:"), sg.I(key="-TEAM_URL-", change_submits=True)],
            [sg.T("Name of CSV file (optional):"), sg.I(key="-CSV_FILE_NAME-", change_submits=True)],
            [sg.T("Choose a path to save CSV: "), sg.I(key="-OS_PATH-", change_submits=True), sg.FolderBrowse()],
            [sg.B("Quick Look"), sg.B("Save"), sg.B("Exit", button_color="tomato")]
    ]"""
